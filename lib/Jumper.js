(function () {
  ...
  Jumper.prototype.draw = function(ctx) {
    ...
    ctx.fillRect(
      ...
      -(this.width / 2), -(this.height / 2), this.width, realHeight
    );
    ctx.fill();
  ...
  Jumper.prototype.isBouncing = function(trampoline) {
    ...
    return (this.position.y + realHeight >= trampoline.pos[1]) //&& (this.position.y <= trampoline.pos[1] - 1);
  };
  ...
  Jumper.prototype.move = function() {
    ...
    if (this.position.y > this.game.trampoline.pos[1] ) {
      this.position.y = this.game.trampoline.pos[1];
    }
  ...
  //     this.speed -= MyTrampoline.Util.gravity;
  //   } else if (this.isCrouching() && this.isBouncing(this.game.trampoline)) {
  //     this.bounce(MyTrampoline.Util.trampolineForce + 1);
  //   } else {
  //     this.bounce(MyTrampoline.Util.trampolineForce);
  //   }
